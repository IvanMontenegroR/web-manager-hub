...
### Los html_tag de los titulos

El CMS arranca cada titulo en **"- Ninguno -"**, y el runner **no elige por vos**: si el
manifiesto no lo dice, el titulo queda sin etiqueta. Es a proposito. El nivel de encabezado
es el **esquema de esa pagina** — se decide cuando se decide que componentes lleva, junto
con el resto del contenido —, no una propiedad del tipo de bloque. Un default que "despues
se corrige" es una decision de estructura tomada por el que no la tenia que tomar.

Asi que va en el manifiesto, un campo mas al lado del titulo:

```json
"field_c_advanced_title": "Paso 1: prepara todo para su llegada",
"field_c_advanced_title.html_tag": "h2"
```

Como guia — no como regla: el titulo de la pagina es el `h1` y hay UNO solo; los titulos
que abren una seccion son `h2`; lo que cuelga de una seccion (las cards de un grid, los
items de un acordeon) es `h3`. Los ejemplos de `manifests/` estan escritos asi.
...
Ahi tambien esta la lista obligatoria que **nace con una fila vacia** (la del Card Grid):
que el contenido caiga en la fila que ya estaba, en vez de dejar una card fantasma al lado.
